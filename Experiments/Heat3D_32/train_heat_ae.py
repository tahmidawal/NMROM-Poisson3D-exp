...
class Conv3DEncoder(nn.Module):
    ...
    pool_size:  int = 4
    dropout_rate: float = 0.1  # Fix 2: Dropout for regularization
    ...
    def __call__(self, x, training: bool = False):
        ...
        for feat in self.features:
            ...
            h = nn.Dropout(rate=self.dropout_rate, deterministic=not training)(h)  # Fix 2
        H, W, D, C = h.shape
        if H != self.pool_size:
...
# 6. Model Init
# ─────────────────────────────────────────
k_dim = 32          # Fix 4: 20 → 32 for more latent capacity
RANK  = 512         # Fix 3: 256 → 512 for multi-Gaussian superpositions
...
NUM_EPOCHS = 20_000  # Fix 5: 8000 → 20000 for convergence
LOG_EVERY  = 2_000

schedule  = optax.warmup_cosine_decay_schedule(
    init_value=0., peak_value=1e-3,
    warmup_steps=1000, decay_steps=NUM_EPOCHS, end_value=1e-5  # Fix 5: warmup 500 → 1000
)
tx        = optax.adamw(learning_rate=schedule, weight_decay=1e-4)  # Fix 3: AdamW with weight decay
opt_state = tx.init(params)

key = jax.random.PRNGKey(1)

# ── Finite-difference gradient energy (for gradient loss) ──────────────
def finite_diff_grad_sq(u_flat):
    """
    Compute sum of squared finite-difference gradients on 3D grid.
    Returns scalar: ||∇u||² ≈ Σ (∂u/∂x)² + (∂u/∂y)² + (∂u/∂z)²
    """
    u = u_flat.reshape((N, N, N))
    # Central differences in interior, forward/backward at boundaries
    dx_u = (u[2:, 1:-1, 1:-1] - u[:-2, 1:-1, 1:-1]) / (2 * dx)
    dy_u = (u[1:-1, 2:, 1:-1] - u[1:-1, :-2, 1:-1]) / (2 * dx)
    dz_u = (u[1:-1, 1:-1, 2:] - u[1:-1, 1:-1, :-2]) / (2 * dx)
    return jnp.sum(dx_u**2) + jnp.sum(dy_u**2) + jnp.sum(dz_u**2)


# ── Per-sample relative loss (Fix 1: normalizes by snapshot energy) ────
GRAD_WEIGHT = 0.01  # Fix 3: Reduced from 0.1 to smooth loss landscape

def per_sample_loss(u_true, u_pred):
    """
    Relative MSE + gradient loss.
    Each snapshot normalized by its own energy → late-time snapshots
    contribute equally to gradients as early-time ones.
    """
    eps = 1e-4
    diff = u_true - u_pred
    
    # Relative MSE: ||u - û||² / ||u||²
    norm_sq = jnp.dot(u_true, u_true) + eps
    rel_mse = jnp.dot(diff, diff) / norm_sq
    
    # Gradient loss: ||∇(u - û)||² / ||∇u||²  (Fix 2: penalizes smearing)
    grad_true = finite_diff_grad_sq(u_true) + eps
    grad_diff = finite_diff_grad_sq(diff)
    grad_loss = grad_diff / grad_true
    
    return rel_mse + GRAD_WEIGHT * grad_loss
...
def train_step(params, batch_stats, opt_state, batch):
    def loss_fn(p):
        ...
        # Fix 1: Relative per-sample loss instead of absolute MSE
    ...
    # Note: Dropout needs a PRNG key when training=True
    _, upd = model.apply(
        {'params': new_params, 'batch_stats': batch_stats},
        batch[0], training=True, mutable=['batch_stats'],
        rngs={'dropout': jax.random.PRNGKey(0)}  # Fixed key OK here — only updating batch_stats
    )
...
def eval_loss(params, batch_stats, batch):
    ...
    # Use same relative loss for eval
    losses = jax.vmap(per_sample_loss)(batch, preds)
    return jnp.mean(losses)
...
for epoch in range(NUM_EPOCHS + 1):
    ...
    # Fix 4: Linear scaling augmentation (heat eq is linear → αu is also a solution)
    key, scale_key = jax.random.split(key)
    scales = jax.random.uniform(scale_key, shape=(BATCH_SIZE, 1), minval=0.5, maxval=2.0)
    aug_batch = batch * scales

    params, batch_stats, opt_state, loss = train_step(
        params, batch_stats, opt_state, aug_batch
    )

    if epoch % LOG_EVERY == 0:
        # Eval on random val subset (no augmentation for eval)
