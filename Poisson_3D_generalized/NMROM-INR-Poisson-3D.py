...
n_test        = 25
test_ks_split = all_ks[:n_test]   # 25 combinations for NMROM benchmark
train_ks      = all_ks            # AE trained on ALL 125, EQ also uses all 125

U_train_list = []
scale_factors_train = []
for i, (k1,k2,k3) in enumerate(train_ks):  # 125 snapshots (all)
...
print(f"   Train cases: {len(train_ks)} (ALL 125), Benchmark cases: {len(test_ks_split)}")
...
for i, (k1,k2,k3) in enumerate(train_ks):  # all 125 snapshots
...
G_train = jnp.concatenate(G_list, axis=0)              # (125*k_dim, num_nodes) on GPU
...
def make_latent_solver(p, V_eq_, b_sp, mask_sp, ug_sp, eq_w, latent_dim):
    ...
    def solve(lat_init, F_eq):
        ...
        def _cond(carry):
            _, gnorm, itr = carry
            return jnp.logical_and(gnorm > 1e-8, itr < 15)
...
# AE trains on ALL 125 snapshots, EQ uses all 125.
# test_ks_split (25 cases): these ARE in training manifold but we test with:
#   (a) 5-NN init (nearby encoded snapshot) -- simulates realistic online use
#   (b) We also collect seen cases (10 sampled from all 125) as diagnostic
# rom_unseen_error: benchmark on test_ks_split with 5-NN init (no exact-match shortcut)
# rom_seen_error:   benchmark on same test_ks_split (same, since all are "seen")
# The test is: can LM-GN find correct z via PDE residual from a nearby (not exact) init?
import random as _random
_rng = _random.Random(99)
seen_sample_ks = _rng.sample(train_ks, min(10, len(train_ks)))

def latent_init_knn(k1, k2, k3, k=5, exclude_exact=True):
    """5-NN inverse-distance weighted latent initialization.
    If exclude_exact=True, skip the exact match (to test ROM, not encoder recall)."""
    dists    = [(k1-a)**2 + (k2-b)**2 + (k3-c)**2 for a,b,c in train_ks]
    sorted_i = np.argsort(dists)
    if exclude_exact and dists[sorted_i[0]] < 1e-12:
        # Exclude the exact match, use next k neighbors
        nn_idx = sorted_i[1:k+1]
    else:
        nn_idx = sorted_i[:k]
    nn_dists = [np.sqrt(dists[j]) + 1e-12 for j in nn_idx]
    weights  = [1.0 / d for d in nn_dists]
    w_sum    = sum(weights)
