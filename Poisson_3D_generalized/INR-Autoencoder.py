...
# Keep seed=42 split for NMROM benchmark consistency, but train AE on ALL 125
random.shuffle(all_ks)
n_test   = 25
test_ks  = all_ks[:n_test]   # 25 held-out combinations (for NMROM benchmark reference only)
train_ks = all_ks            # AE trains on ALL 125 snapshots — manifold covers full space
...
U_train = jnp.stack(U_list)               # (125, num_nodes) — normalized, all k∈{1..5}³
...
# Validation: sample from training set (AE trains on all so no true OOD)
...
print(f"  Training snapshots:   {len(train_ks)} (ALL 125 — covers full parameter space)")
print(f"  Validation snapshots: {len(val_ks)}")
print(f"  NMROM test cases:     {len(test_ks)} (reference only — these ARE in training)")
...
print(f"  Mean test rec error:    {np.mean(test_errs):.4e}  (test_ks -- in training, enc sees them)")
