...
# ==========================================
# 2. Dense Autoencoder (from fast.py - proven architecture)
# ==========================================
class SimpleAutoencoder(nn.Module):
    ...
    def setup(self):
        # Encoder: num_nodes → 1024 → 512 → 256 → latent_dim
        self.enc_dense1 = nn.Dense(1024)
        self.enc_dense2 = nn.Dense(512)
        self.enc_dense3 = nn.Dense(256)
        self.enc_dense4 = nn.Dense(self.latent_dim)
        # Decoder: latent_dim → 256 → 512 → 1024 → num_nodes
        self.dec_dense1 = nn.Dense(256)
        self.dec_dense2 = nn.Dense(512)
        self.dec_dense3 = nn.Dense(1024)
        self.dec_dense4 = nn.Dense(self.num_nodes)
    ...
    def encode(self, x):
        ...
        x = nn.swish(self.enc_dense3(x))
        return self.enc_dense4(x)

    def decode(self, lat):
        h = nn.swish(self.dec_dense1(lat))
        h = nn.swish(self.dec_dense2(h))
        h = nn.swish(self.dec_dense3(h))
        return self.dec_dense4(h)
...
# Generate comprehensive training set: all combinations of k1, k2, k3 in [1, 4]
# This gives 4^3 = 64 combinations - complete coverage of the parameter space
...
# ==========================================
# 6. Hyper-Reduced Solver with Sparse Weight Extraction
# ==========================================
print("\n--- 4. Setting up Hyper-Reduced Solver ---")

# Extract sparse weights from final decoder layer
W_final = params['dec_dense4']['kernel']  # (1024, num_nodes)
b_final = params['dec_dense4']['bias']    # (num_nodes,)
...
def make_latent_solver(p, W_sp, b_sp, mask_sp, ug_sp, eq_w):
    """Factory returning JIT-compiled LM-GN solver with Armijo backtracking."""
    
    def _res_fn(lat, F_eq):
        """Hyper-reduced residual using sparse weights."""
        h = nn.swish(jnp.dot(lat, p['dec_dense1']['kernel']) + p['dec_dense1']['bias'])
        h = nn.swish(jnp.dot(h, p['dec_dense2']['kernel']) + p['dec_dense2']['bias'])
        h = nn.swish(jnp.dot(h, p['dec_dense3']['kernel']) + p['dec_dense3']['bias'])
        u_stencil = (mask_sp * (jnp.dot(h, W_sp) + b_sp) + ug_sp).reshape((-1, 7))
        R = (6 * u_stencil[:, 0]
...
latent_solve = make_latent_solver(params, W_sparse, b_sparse, mask_sparse, u_g_sparse, eq_weights_jnp)
